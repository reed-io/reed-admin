...
JVM_OPTIONS="-server -Xms512m -Xmx2g -XX:MetaspaceSize=256m $GC_OPTS $GC_LOG_OPTS $DUMP_OPTS $OTHER_OPTS"

#使用说明，用来提示输入参数
usage() {
    echo "Usage: sh [appctl].sh [start|stop|restart|status|dump]"
    exit 1
}
...
start(){
  ...
  if [ $? -eq "0" ]; then 
    echo "--- ${JAR_NAME} is already running PID=${pid} ---" 
      ...
      echo "JVM_OPTIONS : "
      echo "$JVM_OPTIONS"
    nohup java -jar $JVM_OPTIONS -Dspring.profiles.active=$PROFILE -Dfitool.app.name=$APP_NAME $APP_HOME/$JAR_NAME >/dev/null 2>&1 &
    echo $! > $PID
